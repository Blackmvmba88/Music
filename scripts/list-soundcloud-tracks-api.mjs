...
const profileUrl = profileInput.startsWith("http") ? profileInput : `https://soundcloud.com/${profileInput}`;
...
async function discoverClientId() {
  ...
  try {
    ...
    if (scriptUrls.length === 0) throw new Error("No se encontraron scripts CDN de SoundCloud en la página principal.");
    ...
    for (const url of scriptUrls.reverse()) {
      try {
        ...
        const idMatch = js.match(/client_id\s*:\s*["']([a-zA-Z0-9]{32})["']/) || js.match(/client_id\s*=\s*["']([a-zA-Z0-9]{32})["']/);
        if (idMatch) {
...
async function resolveProfile(url, clientId, token) {
  ...
  if (!res.ok) throw new Error(`Error de resolución de API de SoundCloud (HTTP ${res.status})`);
  return res.json();
}
...
async function fetchUserTracks(userId, clientId, token) {
  ...
  while (nextUrl) {
    ...
    if (!res.ok) throw new Error(`Error de API de SoundCloud al pedir pistas (HTTP ${res.status})`);
...
async function main() {
  ...
  try {
    ...
    if (!userId) {
      ...
      userUrn = userProfile.urn || (userProfile.id ? `soundcloud:users:${userProfile.id}` : null);
      username = userProfile.username;
      permalinkUrl = userProfile.permalink_url;
      console.log(`👤 Usuario encontrado: "${username}" (${userUrn || `ID: ${userId}`})`);
    }

    const rawTracks = await fetchUserTracks(userId, clientId, oauthToken);
    const formattedTracks = rawTracks.map((item) => ({
      id: item.id ?? null,
      urn: item.urn || (item.id ? `soundcloud:tracks:${item.id}` : null),
      title: item.title,
      ...
      profileAvatarUrl: item.user?.avatar_url || null,
      hasTrackArtwork: Boolean(item.artwork_url),
    }));

    const outputPath = resolve(import.meta.dirname, "..", outputFilename);
    await writeFile(outputPath, JSON.stringify({
      generatedAt: new Date().toISOString(),
      user: { id: userId, urn: userUrn, username, permalinkUrl },
      summary: {
        totalTracks: formattedTracks.length,
        withTrackArtwork: formattedTracks.filter((track) => track.hasTrackArtwork).length,
        missingTrackArtwork: formattedTracks.filter((track) => !track.hasTrackArtwork).length,
      },
    ...
    console.log(`🖼️ Con artwork propio:        ${formattedTracks.filter((track) => track.hasTrackArtwork).length}`);
    console.log(`⬜ Sin artwork propio:        ${formattedTracks.filter((track) => !track.hasTrackArtwork).length}`);
    console.log(`📂 Archivo generado en:       ${outputPath}`);
    if (formattedTracks.length > 0) {
      console.log("\n📋 Primeras 10 pistas encontradas:");
      console.table(formattedTracks.slice(0, 10).map((track) => ({
        URN: track.urn,
        Título: track.title.length > 35 ? `${track.title.slice(0, 32)}...` : track.title,
        Duración: track.durationFormatted,
        Portada: track.hasTrackArtwork ? "SÍ" : "NO",
        Visibilidad: String(track.sharing || "").toUpperCase(),
      })));
    }
  } catch (error) {
